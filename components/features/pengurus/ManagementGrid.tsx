...
export function ManagementGrid({ className = '', initialData }: ManagementGridProps) {
  ...
  return (
    <section className={`relative py-12 px-5 md:py-24 md:px-8 bg-gray-50 overflow-hidden ${className}`}>
      ...
      {/* Decorative elements */}
      <div className="hidden md:block absolute top-32 left-1/4 w-10 h-10 bg-iark-yellow rounded-full opacity-20 animate-pulse-slow" />
      <div className="hidden md:block absolute top-2/3 right-16 w-8 h-8 bg-iark-red rounded-full opacity-20 animate-drift" />
      <div className="hidden md:block absolute bottom-1/4 left-20 w-12 h-12 bg-iark-blue rounded-full opacity-30 animate-pulse-slow" />
