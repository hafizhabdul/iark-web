...
export function TestimoniSection({ className = '', initialData }: TestimoniSectionProps) {
  ...
  return (
    <section className={`relative py-12 px-5 md:py-24 md:px-8 bg-white overflow-hidden ${className}`}>
      ...
      <div className="absolute top-0 left-0 w-48 h-48 md:w-96 md:h-96 bg-iark-yellow/5 rounded-full blur-3xl" />
      <div className="absolute bottom-0 right-0 w-48 h-48 md:w-96 md:h-96 bg-iark-blue/5 rounded-full blur-3xl" />

      {/* Decorative small elements with animations */}
      <div className="hidden md:block absolute top-32 right-1/3 w-10 h-10 bg-iark-red rounded-full opacity-20 animate-pulse-slow" />
      <div className="hidden md:block absolute top-2/3 left-16 w-8 h-8 bg-iark-yellow rounded-full opacity-20 animate-drift" />
      <div className="hidden md:block absolute bottom-1/4 right-20 w-12 h-12 bg-iark-blue rounded-full opacity-30 animate-pulse-slow" />
            ...
            {stories.map((story) => (
              ...
              >
                ...
                <div className="p-4 md:p-6">
                  <h3 className="text-lg md:text-xl font-bold text-iark-black mb-2 line-clamp-2 group-hover:text-iark-red transition-colors">
                    {story.title}
                  </h3>
                  {story.excerpt && (
